...
class LMTrainDataset(Dataset):
    ...
    def _prepare_distillation_pair(self, student, record):
        ...
        if self.needs_self_distill_context:
            ...
            # The reference context comes from the full original response,
            # independent of the response newly generated by the student.
            canonical_response = student["response"].replace("\r\n", "\n").replace("\r", "\n")
            student["self_distill_steps"] = [
                part for part in canonical_response.split(self.separator) if part.strip()]
