...
class TalasPR(nn.Module):
    def __init__(self, args):
        super(TalasPR, self).__init__()
        self.args = args
    ...
    def participation_ratio_erank(self, z_list_first: list[torch.Tensor], 
        ...
        def _participation_ratio(z_padded, mask, lengths):
            ...
            # Ma trận hiệp phương sai chính xác: Cov = z_c^T @ z_c / (n-1)  -> [B, D, D]
            Cov = torch.bmm(z_c.transpose(1, 2), z_c) / (n_valid - 1.0).view(-1, 1, 1)

            # trace(Cov) = tổng đường chéo
            trace1 = torch.diagonal(Cov, dim1=-2, dim2=-1).sum(dim=-1)                 # [B]

            # trace(Cov^2) = ||Cov||_F^2 = sum_{i,j} Cov_ij^2  (vì Cov đối xứng)
            trace2 = (Cov ** 2).sum(dim=(-2, -1))                                      # [B]

            pr = trace1.pow(2) / trace2.clamp_min(eps)                                 # [B] participation ratio chính xác
    ...
    def _compute_modality_distill(self, student_hidden_states, image_features, 
        ...
        if len(stu_img_tokens[last_layer_idx]) > 0:
            ...
            for l in layers[1:-1]:
                ...
                sigreg_erank_loss += self.participation_ratio_erank(stu_img_tokens[0], 
                                                                    stu_img_tokens[l])
