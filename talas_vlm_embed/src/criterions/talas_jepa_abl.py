...
class TalasJepaAbl(nn.Module):
    def __init__(self, args):
        super(TalasJepaAbl, self).__init__()
        self.args = args
    ...
    def _compute_modality_distill(self, student_hidden_states, image_features, 
        ...
        if 0 in layer_mapping and last_layer_idx in layer_mapping:
            ...
            layers = [0, 5, int(last_layer_idx / 2), int(4 * last_layer_idx / 5), last_layer_idx]
        ...
        if len(stu_img_tokens[last_layer_idx]) > 0:
            ...
            for l in layers[1:-1]:
                k_layers += 1
                sigreg_erank_loss += self.sketched_participation_ratio_erank(stu_img_tokens[5], 
                                                                             stu_img_tokens[l], 
