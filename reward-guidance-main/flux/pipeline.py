...
from reward_guidance_math import stabilize_low_rank_curvatures
...
class GuidedFluxPipeline(FluxPipeline):
    ...
    def _compute_grad_second_order(
        ...
        with torch.enable_grad():
            ...
            # The old implementation retained only the most-negative
            # eigenvalues, silently deleting positive curvature.  For
            # exp(+beta*r), both signs belong in I - beta*variance*H. Select
            # the largest-magnitude directions and stabilize positive ones
            # only if the local precision would become singular.
            k = min(max(int(k_eig), 0), H_g.shape[-1])
            selected = torch.topk(evals.abs(), k=k, largest=True).indices
            mu = evals[selected]
            U = evecs[:, selected]
            mask = mu.abs() > 1e-6
            mu = mu[mask]
            ...
            if W_cols and correction_scale > 0.0:
                ...
                with torch.no_grad():
                    ...
                    mu_math, precision_diagnostics = stabilize_low_rank_curvatures(
                        W_math.T @ W_math,
                        mu.to(device=W.device, dtype=math_dtype),
                        scale=c,
                        precision_floor=precision_floor,
                    )
                    active = mu_math.abs() > 1e-12
                    W_math = W_math[:, active]
                    mu_math = mu_math[active]
                    if verbose:
                        strength = (
                            c * W_math.norm().item() ** 2 * mu_math.abs().max().item()
                        )
                        print(
                            f"k_eig={k_eig} W_cols={W_math.shape[1]} "
                            f"correction_strength={strength:.3e} "
                            f"min_precision="
                            f"{precision_diagnostics.min_eigenvalue_before:.3e}->"
                            f"{precision_diagnostics.min_eigenvalue_after:.3e} "
                            f"positive_scale="
                            f"{precision_diagnostics.positive_curvature_scale:.3e}"
                        )
    ...
    def _compute_grad_k_particles(
        ...
        """k-particle plug-in: ∇log ĥ ≈ Σ_i softmax(β r_i) β∇r_i.
        ...
        log_w = reward_scale * rewards_tensor
    ...
    def __call__(
        ...
        second_order_precision_floor: float = 0.05,
        verbose: bool = False,
    ):
        ...
        if lam < 0.0 or reward_scale < 0.0:
            raise ValueError("lam and reward_scale must be non-negative.")
        if second_order_rank < 1:
            raise ValueError("second_order_rank must be positive.")
        if not 0.0 < second_order_precision_floor <= 1.0:
            raise ValueError("second_order_precision_floor must lie in (0, 1].")

        # ``beta`` is the single coefficient of exp(+beta*r). The legacy CLI
        # used reward_scale*lam in second-order, reward_scale in k=1, and lam
        # only in particle weights. Preserve old commands via this mapping,
        # while making all methods consistent when beta is supplied.
        beta_eff = float(beta if beta is not None else reward_scale * lam)
        if method == "second_order" and reward_fn is not None and beta_eff != 0.0:
            if not getattr(reward_fn, "supports_features", False):
                raise ValueError(
        ...
        is_guided_run = (
            reward_fn is not None and beta_eff != 0.0 and num_guidance_steps > 0
        )
            ...
            if is_guided_run and guidance_start_idx <= i < guidance_end_idx:
                if method == "second_order":
                    grad, reward_value, raw_grad_norm = self._compute_grad_second_order(
                        ...
                        grad_divisor,
                        lam=1.0,
                        k_eig=second_order_rank,
                        precision_floor=second_order_precision_floor,
                        verbose=verbose,
                    )
                ...
                if sigma_damp is not None:
                    damp_factor = damped_lambda_factor(sigma, sigma_damp, beta_eff)
