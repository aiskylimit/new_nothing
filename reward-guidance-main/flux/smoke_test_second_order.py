...
def bind_linear_methods(pipe):
    ...
    LinearPipe._compute_grad_second_order = (
        GuidedFluxPipeline._compute_grad_second_order
    )
    return pipe
...
def make_mixed_curvature_reward():
    """Quadratic reward with one strong positive and two negative directions."""

    linear = torch.tensor([0.7, -0.2, 0.4], dtype=torch.float32)
    hessian = torch.diag(torch.tensor([20.0, -2.0, -1.0], dtype=torch.float32))

    def fn(image, return_features=False):
        z = image.mean(dim=(2, 3))

        def head_fn(z_in):
            linear_term = (z_in * linear.to(z_in)).sum(dim=-1)
            quadratic = 0.5 * torch.einsum("bi,ij,bj->b", z_in, hessian.to(z_in), z_in)
            return linear_term + quadratic
...
def t_second_order_matches_dense_solve():
    ...
    feature = (
        scale * latents
        + math.sqrt(max(sigma_prime**2 - scale**2 * sigma**2, 1e-8)) * eps
    ) @ pipe.A.T
...
def t_mixed_curvature_is_stable_and_matches_dense_solve():
    """Positive curvature is retained, stabilized, and matches a dense solve."""

    from pipeline import posterior_variance
    from reward_guidance_math import stabilize_low_rank_curvatures

    pipe = bind_linear_methods(LinearPipe())
    reward_fn = make_mixed_curvature_reward()
    latents = torch.tensor([[[0.3, -0.4, 0.2]]], dtype=torch.float32)
    args = make_linear_args(latents)
    seed = 29
    lam = 1.0
    ...
    jacobian = scale * pipe.A
    hessian = torch.diag(torch.tensor([20.0, -2.0, -1.0]))
    linear = torch.tensor([0.7, -0.2, 0.4])
    feature_grad = linear + hessian @ feature.flatten()
    beta = args["reward_scale"] * lam
    first_order = beta * (jacobian.T @ feature_grad)
    c = beta * posterior_variance(sigma_prime)

    evals, evecs = torch.linalg.eigh(hessian.double())
    selected = torch.topk(evals.abs(), k=3).indices
    mu = evals[selected]
    U = evecs[:, selected]
    W = jacobian.double().T @ U
    stabilized, diagnostics = stabilize_low_rank_curvatures(
        W.T @ W, mu, c, precision_floor
    )
    stable_hessian = U @ torch.diag(stabilized) @ U.T
    dense_matrix = torch.eye(3, dtype=torch.float64) - (
        c * jacobian.double().T @ stable_hessian @ jacobian.double()
    )
    expected = torch.linalg.solve(dense_matrix, first_order.double()).view_as(actual)

    error = (actual.double() - expected).abs().max().item()
    assert diagnostics.positive_curvature_scale < 1.0
    assert diagnostics.min_eigenvalue_after >= precision_floor - 1e-6
    # The production path intentionally solves in fp32 when activations are
    # bf16/fp16; allow the corresponding rounding near the precision floor.
    assert error < 5e-5, f"stabilized Woodbury != dense solve: {error:.3e}"
    print(
        f"  positive_scale={diagnostics.positive_curvature_scale:.3e} "
        f"min_precision={diagnostics.min_eigenvalue_after:.3e} "
...
if __name__ == "__main__":
    ...
    check(
        "16. curvature hon hop on dinh va khop nghiem dense",
        t_mixed_curvature_is_stable_and_matches_dense_solve,
    )
