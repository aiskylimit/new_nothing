...
def _write_metadata(args, cache_dir: str):
    ...
    beta_eff = args.beta if args.beta is not None else args.reward_scale * args.lam
    beta_source = (
        "explicit --beta" if args.beta is not None else "legacy reward_scale*lam"
    )
    lines.append(f"beta:                   {beta_eff:g}  ({beta_source})")
    ...
    if args.sigma_damp is not None:
        lines.append(
            ...
            f"(β_t = β/(1 + 2β σ²_{{1|t}}); heavier damping near noise)"
        )
    if args.num_particles > 1:
        lines.append(
            f"num_particles:          {args.num_particles}  "
            f"(plug-in k; ∇log ĥ ≈ Σ softmax(β r_i) β∇r_i)"
        )
    if args.num_particles > 1 or args.method == "second_order":
        lines.append(
            f"lam:                    {args.lam:g}  "
            f"(legacy beta multiplier, ignored when --beta is explicit)"
        )
    if args.method == "second_order":
        lines.append(f"second_order_rank:      {args.second_order_rank}")
        lines.append(f"precision_floor:        {args.second_order_precision_floor:g}")
...
def validate_args(args):
    ...
    if args.reward_scale < 0.0 or args.lam < 0.0:
        raise ValueError("--reward-scale and --lam must be non-negative.")
    if args.second_order_rank < 1:
        raise ValueError("--second-order-rank must be positive.")
    if not 0.0 < args.second_order_precision_floor <= 1.0:
        raise ValueError("--second-order-precision-floor must lie in (0, 1].")
    if args.method == "second_order":
        ...
        beta_eff = args.beta if args.beta is not None else args.reward_scale * args.lam
        if beta_eff <= 0.0 and args.reward_scale != 0.0:
            raise ValueError("The effective --beta must be positive for guidance.")
...
def main(args):
    ...
    if beta_eff != 0.0:
        cache_dir += f"_beta{beta_eff:g}"
    if args.method == "second_order":
        cache_dir += (
            f"_rank{args.second_order_rank}"
            f"_floor{args.second_order_precision_floor:g}"
        )
    if args.sigma_damp is not None:
        cache_dir += f"_damp{args.sigma_damp:g}"
    if args.num_particles > 1:
        cache_dir += f"_k{args.num_particles}"
    cache_dir += (
        f"_seed{args.seed}_n{args.num_images}"
        f"_{args.width}x{args.height}_infer{args.num_steps}"
    )
    ...
    for i in range(args.num_images):
        ...
        out = pipe(
            ...
            beta=args.beta,
            method=args.method,
            second_order_rank=args.second_order_rank,
            second_order_precision_floor=args.second_order_precision_floor,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "guidance becomes Σ softmax(β r_i) β∇r_i where each "
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help=("Legacy multiplier in beta=reward_scale*lam. Prefer explicit --beta."),
    )
    parser.add_argument(
        "--beta",
        type=float,
        default=None,
        help=(
            "Coefficient of exp(+beta*r), shared by every guidance method. "
            "If omitted, legacy reward_scale*lam is used."
        ),
    )
    parser.add_argument(
        "--second-order-rank",
        type=int,
        default=16,
        help="Number of largest-absolute feature-Hessian directions.",
    )
    parser.add_argument(
        "--second-order-precision-floor",
        type=float,
        default=0.05,
        help="Minimum normalized precision eigenvalue after stabilization.",
    )
