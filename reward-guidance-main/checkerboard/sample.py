...
from reward_guidance_math import second_order_preconditioned_gradient
...
def compute_first_order_guidance(
    velocity_net, t, x, lam, reward_center, sigma_r, device="cpu"
):
    """First-order positive-reward guidance at deterministic ``E[X_1|X_t]``."""

    x_input = x.detach().requires_grad_(True)
    t_tensor = torch.full((len(x_input),), t, device=device, dtype=x_input.dtype)
    mu = denoiser(velocity_net, t_tensor, x_input)
    objective = lam * reward_fn(mu, reward_center, sigma_r).sum()
    return torch.autograd.grad(objective, x_input)[0].detach()
...
def compute_second_order_guidance(
    ...
    Uses a second-order Taylor expansion around the deterministic denoiser
    estimate of ``E[X_1 | X_t=x]``.  Positive reward curvature is retained and
    clipped only when needed to keep ``I - beta * variance * H`` positive.
    ...
    Args:
        ...
        glass_steps: retained for CLI compatibility; not used by this deterministic method
        device: torch device
    Returns:
    ...
    # A stochastic Taylor centre makes repeated evaluations disagree and adds
    # Monte-Carlo noise to the Hessian approximation. The denoiser is the
    # conditional-mean estimator represented by the flow model.
    t_tensor = torch.full((B,), t, device=device, dtype=x_input.dtype)
    mu = denoiser(velocity_net, t_tensor, x_input)
    ...
    with torch.no_grad():
        ...
        # 3. Correct positive-reward Laplace factor:
        #    (I - beta * variance * H)^-1 (beta * grad r).
        precond_grad, diagnostics = second_order_preconditioned_gradient(
            grad_r_ana,
            H_r_ana,
            variance=sigma_t_sq,
            beta=lam,
        ...
        if verbose and diagnostics.num_clipped_curvatures:
            print(
                "second-order precision stabilization: "
                f"min {diagnostics.min_eigenvalue_before:.3e} -> "
                f"{diagnostics.min_eigenvalue_after:.3e}, "
                f"clipped={diagnostics.num_clipped_curvatures}",
                flush=True,
            )
...
def sample_guided(
    ...
    Args:
        ...
        method: ``plugin``, ``fmrg``, ``first_order`` or ``second_order``
        ...
        for step in trange(num_ode_steps, desc=f"Guided ODE ({method})"):
            ...
            with torch.enable_grad():
                if method == "fmrg":
                    ...
                    guidance = compute_first_order_guidance(
                        velocity_net,
                        t if t > 0 else dt,
                        x,
                        lam_t,
...
def sample_unguided(
    ...
    """Generate unguided samples from the base flow using Heun's method."""
    velocity_net.eval()
...
def _cache_path(output_dir, k, lam, sigma_damp=None, method="plugin"):
    suffix = f"_damp{sigma_damp}" if sigma_damp is not None else ""
    method_str = method if method != "plugin" else f"k{k}"
    return os.path.join(output_dir, f"guided_{method_str}_lam{lam}{suffix}.npz")


def load_cache(output_dir, k, lam, cache_key, sigma_damp=None, method="plugin"):
    """Load a cache only when every generation-defining input still matches."""

    path = _cache_path(output_dir, k, lam, sigma_damp, method)
    if os.path.exists(path):
        data = np.load(path)
        cached_key = str(data["cache_key"].item()) if "cache_key" in data else None
        if cached_key == cache_key:
            return torch.tensor(data["samples"]), torch.tensor(data["rewards"])
        print(f"Ignoring stale cache {path}", flush=True)
    return None, None


def save_cache(
    output_dir, k, lam, samples, rewards, sigma_damp, cache_key, method="plugin"
):
...
def _model_fingerprint(checkpoint_path, use_ema):
    stat = os.stat(checkpoint_path)
    state_name = "ema" if use_ema else "raw"
    return f"{os.path.abspath(checkpoint_path)}:{stat.st_size}:{stat.st_mtime_ns}:{state_name}"
...
def generate_analytic_tilt(args):
    ...
    analytic_key = (
        f"v{CACHE_VERSION}|n={args.num_samples}|beta={args.lam}|"
        f"center={tuple(args.reward_center)}|sigma_r={args.sigma_r}|seed={args.seed}"
    )
    if os.path.exists(path):
        cached = np.load(path)
        cached_key = str(cached["cache_key"].item()) if "cache_key" in cached else None
        if cached_key == analytic_key and len(cached["samples"]) >= args.num_samples:
    ...
    np.savez(path, samples=samples, rewards=rewards, cache_key=np.array(analytic_key))
...
def main(args):
    ...
    # Load trained model
    checkpoint_path = os.path.join(args.model_dir, "velocity_net.pt")
    ...
    use_ema = "ema_model" in checkpoint
    ...
    model_fingerprint = _model_fingerprint(checkpoint_path, use_ema)
    base_key = (
        f"v{CACHE_VERSION}|model={model_fingerprint}|seed={args.seed}|"
        f"steps={args.num_ode_steps}|rescale={rescale}"
    )
    unguided_path = os.path.join(args.output_dir, "unguided.npz")
    if os.path.exists(unguided_path):
        data = np.load(unguided_path)
        cached_key = str(data["cache_key"].item()) if "cache_key" in data else None
        if cached_key == base_key and len(data["samples"]) >= args.num_samples:
            unguided_samples = torch.tensor(data["samples"][: args.num_samples])
            print(f"Loaded {unguided_samples.shape[0]} unguided samples from cache.")
        else:
            unguided_samples = None
    else:
        unguided_samples = None
    if unguided_samples is None:
        ...
        np.savez(
            unguided_path,
            samples=unguided_samples.numpy(),
            cache_key=np.array(base_key),
        )
    ...
    guided_key = (
        f"{base_key}|method={args.method}|k={args.k}|beta={args.lam}|"
        f"reward_center={tuple(args.reward_center)}|sigma_r={args.sigma_r}|"
        f"glass={args.glass_steps}|schedule={args.sigma_schedule}|"
        f"fmrg={args.fmrg_inner_steps}|floor={args.precision_floor}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Positive-reward tilt coefficient; overrides legacy --lam.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="plugin",
        choices=["plugin", "fmrg", "first_order", "second_order"],
        help="Guidance method: 'plugin' (k-particle GLASS plug-in), 'fmrg' "
        "(flow map trajectory guidance), 'first_order', or 'second_order'",
    )
    parser.add_argument(
        "--fmrg-inner-steps",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-4,
        help="Minimum eigenvalue of the normalized second-order precision.",
    )
    ...
    if args.beta is not None:
        args.lam = args.beta
    if args.lam < 0.0:
        parser.error("--beta/--lam must be non-negative.")
    if args.sigma_r <= 0.0:
        parser.error("--sigma-r must be positive.")
    if args.num_samples < 1 or args.num_ode_steps < 1 or args.glass_steps < 1:
        parser.error("sample counts and integration-step counts must be positive.")
    if not 0.0 < args.precision_floor <= 1.0:
        parser.error("--precision-floor must lie in (0, 1].")
