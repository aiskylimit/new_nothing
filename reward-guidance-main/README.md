...
Experiment code for reward-guided flow matching. The repository implements
direct reward-tilted endpoint training, first- and second-order guidance,
plug-in (Doob *h*-transform) sampling, reward damping, best-of-*n*, and flow map
reward guidance (FMRG). The common target is
`p_beta(x|c) ∝ p_data(x|c) exp(+beta r(x,c))`; see
`docs/REWARD_GUIDED_FLOW_MATCHING.md` for the signs and approximations.
...
# Checkerboard
cd checkerboard/
python train.py --num-steps 500000 --target-distribution base
python train.py --num-steps 100000 --target-distribution reward_tilted --beta 10 \
    --init-checkpoint results/velocity_net.pt --output-dir results/reward_tilted_beta10
python sample.py --analytic-tilt --beta 10.0
python sample.py --method plugin --k 1 --beta 10.0 --num-samples 20000
python sample.py --method first_order --beta 10.0 --num-samples 20000
python sample.py --method second_order --beta 10.0 --num-samples 20000
python evaluate_reward_tilted.py --model-dir results/reward_tilted_beta10 --beta 10
...
│   ├── train.py                      # base and direct reward-tilted flow training
│   ├── sample.py                     # plug-in, first/second-order, FMRG samplers
│   ├── evaluate_reward_tilted.py     # exact-target distribution metrics
│   ├── make_reward_guidance_comparison.py # five-way generated-sample grid
...
# (1) Train the base velocity field and the direct reward-tilted flow.
python train.py --num-steps 500000 --target-distribution base
python train.py --num-steps 100000 --target-distribution reward_tilted --beta 10 \
  --init-checkpoint results/velocity_net.pt \
  --output-dir results/reward_tilted_beta10
...
python sample.py \
    ...
    --beta 1                              # coefficient of exp(+beta * reward)
    [--num-particles 8]                  # for k = 8 conditions
...
Reward: `imagereward`. `--prompt` controls the base generation, whereas
`--ir-prompt` defines the preference being optimized. Keeping them separate is
important for the market stress test.

| Figure | Prompt | Damped sigma |
|---|---|---|
| `imagereward_archaeologist` | *"Photorealistic close-up of an archaeologist brushing weathered paper talismans in a flooded bunker, flashlight haze, focused expression"* | 0.15 |
| `imagereward_miner`         | *"Cinematic split diorama of a miner above and below ground, helmet beam, coal dust, family dinner fading opposite"* | 0.10 |
| `imagereward_market`        | generation: dull/desaturated market; reward: *"a vibrant Indian outdoor market with colorful stalls and produce"* | 0.05 |
