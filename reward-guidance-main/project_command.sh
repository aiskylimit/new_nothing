...
CHECKERBOARD_TILT_BETA="${CHECKERBOARD_TILT_BETA:-10}"
...
UV_PROJECT_ENVIRONMENT="$PROJECT_ENV" uv sync --frozen
source "$PROJECT_ENV/bin/activate"
python -c "import diffusers, torch, transformers"

# Fail fast on mathematical/sign regressions and low-rank runtime errors before
# allocating H200 time or downloading model weights.
...
if [ ! -f "results/velocity_net.pt" ]; then
    echo "  Training base flow (${CHECKERBOARD_BASE_STEPS} steps)..."
    python train.py --num-steps "$CHECKERBOARD_BASE_STEPS" \
        --target-distribution base --output-dir results
else
    echo "  Checkpoint đã tồn tại, skip training."
fi

TILTED_DIR="results/reward_tilted_beta${CHECKERBOARD_TILT_BETA}"
if [ ! -f "$TILTED_DIR/velocity_net.pt" ]; then
    echo "  Fine-tuning direct reward-tilted flow (${CHECKERBOARD_TILT_STEPS} steps)..."
    python train.py --num-steps "$CHECKERBOARD_TILT_STEPS" \
        --target-distribution reward_tilted --beta "$CHECKERBOARD_TILT_BETA" \
        --init-checkpoint results/velocity_net.pt --output-dir "$TILTED_DIR"
else
    echo "  Reward-tilted checkpoint đã tồn tại, skip fine-tuning."
fi

python sample.py --analytic-tilt --beta "$CHECKERBOARD_TILT_BETA"
python sample.py --method plugin --k 1 --beta "$CHECKERBOARD_TILT_BETA" --num-samples 20000
python sample.py --method first_order --beta "$CHECKERBOARD_TILT_BETA" --num-samples 20000
python sample.py --method second_order --beta "$CHECKERBOARD_TILT_BETA" --num-samples 20000
python sample.py --method plugin --k 8 --beta "$CHECKERBOARD_TILT_BETA" --num-samples 5000
python sample.py --method plugin --k 1 --beta "$CHECKERBOARD_TILT_BETA" \
    --sigma-damp 0.2 --num-samples 20000
python evaluate_reward_tilted.py --model-dir "$TILTED_DIR" \
    --beta "$CHECKERBOARD_TILT_BETA" --num-samples 20000
python make_reward_guidance_comparison.py --beta "$CHECKERBOARD_TILT_BETA" \
    --tilted-model-dir "$TILTED_DIR"
python make_main_figure.py
python plot.py --plots bon_paper --lam "$CHECKERBOARD_TILT_BETA"
cd ..
...
run_flux blueness_fox gns100 \
    --reward blue_minus_rg --prompt "$FOX_PROMPT" \
    --beta 1.0 --gradient-norm-scale 100 $FLUX_COMMON

run_flux blueness_fox gns50 \
    --reward blue_minus_rg --prompt "$FOX_PROMPT" \
    --beta 1.0 --gradient-norm-scale 50 $FLUX_COMMON

run_flux blueness_fox gns50_k8 \
    --reward blue_minus_rg --prompt "$FOX_PROMPT" \
    --beta 1.0 --gradient-norm-scale 50 --num-particles 8 $FLUX_COMMON
...
ARCH_GEN_PROMPT="Photorealistic close-up of an archaeologist brushing weathered paper talismans in a flooded bunker, flashlight haze, focused expression"
ARCH_REWARD_PROMPT="$ARCH_GEN_PROMPT"

run_flux imagereward_archaeologist unguided \
    --reward imagereward --prompt "$ARCH_GEN_PROMPT" --ir-prompt "$ARCH_REWARD_PROMPT" \
    --reward-scale 0 $FLUX_COMMON
...
run_flux imagereward_archaeologist gns100_damp0.15 \
    --reward imagereward --prompt "$ARCH_GEN_PROMPT" --ir-prompt "$ARCH_REWARD_PROMPT" \
    --beta 1.0 --gradient-norm-scale 100 --sigma-damp 0.15 $FLUX_COMMON
...
MINER_GEN_PROMPT="Cinematic split diorama of a miner above and below ground, helmet beam, coal dust, family dinner fading opposite"
MINER_REWARD_PROMPT="$MINER_GEN_PROMPT"

run_flux imagereward_miner unguided \
    --reward imagereward --prompt "$MINER_GEN_PROMPT" --ir-prompt "$MINER_REWARD_PROMPT" \
    --reward-scale 0 $FLUX_COMMON
...
run_flux imagereward_miner gns100_damp0.10 \
    --reward imagereward --prompt "$MINER_GEN_PROMPT" --ir-prompt "$MINER_REWARD_PROMPT" \
    --beta 1.0 --gradient-norm-scale 100 --sigma-damp 0.10 $FLUX_COMMON
...
MARKET_GEN_PROMPT="a dull, muted, washed-out, desaturated Indian outdoor market with stalls and produce, overcast gloomy lighting, faded colors"
MARKET_REWARD_PROMPT="a vibrant Indian outdoor market with colorful stalls and produce"

run_flux imagereward_market unguided \
    --reward imagereward --prompt "$MARKET_GEN_PROMPT" --ir-prompt "$MARKET_REWARD_PROMPT" \
    --reward-scale 0 $FLUX_COMMON
...
run_flux imagereward_market gns30 \
    --reward imagereward --prompt "$MARKET_GEN_PROMPT" --ir-prompt "$MARKET_REWARD_PROMPT" \
    --beta 1.0 --gradient-norm-scale 30 $FLUX_COMMON
...
run_flux imagereward_market gns100_damp0.05 \
    --reward imagereward --prompt "$MARKET_GEN_PROMPT" --ir-prompt "$MARKET_REWARD_PROMPT" \
    --beta 1.0 --gradient-norm-scale 100 --sigma-damp 0.05 $FLUX_COMMON
...
for LABEL_PROMPT in \
    "archaeologist|$ARCH_GEN_PROMPT|$ARCH_REWARD_PROMPT" \
    "miner|$MINER_GEN_PROMPT|$MINER_REWARD_PROMPT" \
    "market|$MARKET_GEN_PROMPT|$MARKET_REWARD_PROMPT"; do

    IFS='|' read -r LABEL GEN_PROMPT REWARD_PROMPT <<< "$LABEL_PROMPT"

    # 6a. Second-Order, GNS=50 (apple-to-apple with plugin GNS=50)
    run_flux "imagereward_${LABEL}" "2nd_order_gns50" \
        --reward imagereward --prompt "$GEN_PROMPT" --ir-prompt "$REWARD_PROMPT" \
        --method second_order --beta 1.0 --gradient-norm-scale 50 $FLUX_COMMON

    # 6b. Second-Order, GNS=100
    run_flux "imagereward_${LABEL}" "2nd_order_gns100" \
        --reward imagereward --prompt "$GEN_PROMPT" --ir-prompt "$REWARD_PROMPT" \
        --method second_order --beta 1.0 --gradient-norm-scale 100 $FLUX_COMMON

    # 6c. Second-Order, Unnormalized (automatic Woodbury damping)
    run_flux "imagereward_${LABEL}" "2nd_order_unnorm" \
        --reward imagereward --prompt "$GEN_PROMPT" --ir-prompt "$REWARD_PROMPT" \
        --method second_order --beta 1.0 --gradient-norm-scale 0.0 $FLUX_COMMON

    # 6d. Second-Order + Bo4: sinh 80 ảnh, pick top 20
    run_flux "imagereward_${LABEL}" "2nd_order_bo4_raw" \
        --reward imagereward --prompt "$GEN_PROMPT" --ir-prompt "$REWARD_PROMPT" \
        --method second_order --beta 1.0 --gradient-norm-scale 50 \
...
# Independent text-image alignment/diversity evaluation. ImageReward is the
# guidance metric; CLIP is deliberately held out from generation.
EVAL_GPU="${SELECTED_GPU_IDS[0]//[[:space:]]/}"
CUDA_VISIBLE_DEVICES="$EVAL_GPU" python evaluate_image_results.py \
    --data-root ../data --device cuda \
    --output ../data/imagereward_evaluation.json
    ...
    for root, dirs, files in os.walk(base):
        if 'rewards.npy' in files and 'bo4_raw' not in root:
            ...
            summary[key] = {
                ...
                'ci95': [
                    float(np.mean(r) - 1.96 * np.std(r, ddof=1) / np.sqrt(len(r))),
                    float(np.mean(r) + 1.96 * np.std(r, ddof=1) / np.sqrt(len(r))),
                ] if len(r) > 1 else [float(np.mean(r)), float(np.mean(r))],
                'n': int(len(r)),
            }
...
cp data/imagereward_evaluation.json "$EXPORT_DIR/"
